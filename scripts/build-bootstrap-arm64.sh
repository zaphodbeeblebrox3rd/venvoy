...
# Build ARM64-only bootstrap image for venvoy
...
echo "🔨 Building ARM64-only venvoy bootstrap image..."
...
# Build bootstrap image (ARM64 only)
echo "📦 Building bootstrap image for ARM64..."
...
if [ "$CONTAINER_RUNTIME" = "docker" ]; then
    docker buildx build \
        ...
        -t ${REGISTRY}/${IMAGE_NAME}:bootstrap-arm64 \
        --push \
        .
    
    echo "✅ Bootstrap image (ARM64) built and pushed successfully!"
    ...
    if podman build \
        ...
        -t ${REGISTRY}/${IMAGE_NAME}:bootstrap-arm64 \
        .; then
        echo "📤 Pushing ${REGISTRY}/${IMAGE_NAME}:bootstrap-arm64..."
        if podman push ${REGISTRY}/${IMAGE_NAME}:bootstrap-arm64; then
            echo "✅ Bootstrap image (ARM64) built and pushed successfully!"
        else

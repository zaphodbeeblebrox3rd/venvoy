...
class DockerManager:
    ...
    def ensure_docker_installed(self):
        ...
        if not DOCKER_AVAILABLE:
            raise RuntimeError(
                "Docker Python module not available. Please install with: pip install docker"
            )
    ...
    def _prompt_editor_choice(self) -> tuple[str, bool]:
        ...
        while True:
            choice = input(
                "\nWhich editor would you prefer? (1 for Cursor, 2 for VSCode): "
            ).strip()
            if choice == "1":
                print(
                    "🧠 Excellent choice! Cursor provides cutting-edge AI assistance."
                )
                return ("cursor", True)
            elif choice == "2":
        ...
        while True:
            ...
            if choice == "1":
                ...
                if success and self.platform._check_cursor_available():
                    ...
                    print(
                        "💡 Don't worry - venvoy will work with an interactive shell instead."
                    )
                    return ("none", False)
            elif choice == "2":
                ...
                print(
                    "📝 No problem! Venvoy will use an enhanced interactive shell with AI-ready environment."
                )
                return ("none", False)
            else:
        ...
        try:
            if system == "windows":
                ...
                if shutil.which("winget"):
                    try:
                        subprocess.run(
                            ["winget", "install", "Cursor.Cursor"], check=True
                        )
                        return True
                    except subprocess.CalledProcessError:
                        pass
                ...
                if shutil.which("brew"):
                    ...
                    try:
                        subprocess.run(
                            ["brew", "install", "--cask", "cursor"], check=True
                        )
                        return True
                    except subprocess.CalledProcessError:
                        pass
        ...
        try:
            if system == "windows":
                ...
                if shutil.which("snap"):
                    ...
                    try:
                        subprocess.run(
                            ["sudo", "snap", "install", "code", "--classic"], check=True
                        )
                        return True
                    except subprocess.CalledProcessError:
                        pass
    ...
    def push_image(self, tag: str):
        ...
        try:
            subprocess.run(
                ["docker", "buildx", "build", "--push", "--tag", tag], check=True
            )
        except subprocess.CalledProcessError as e:
